<script type="text/ecmascript-6">
    ...
    export default {
         ...
         */
        mounted() {
            document.title = this.title + " - Microscope";
        ...
        methods: {
            loadEntries(after){
                axios.get(Microscope.apiPath + this.resource +
                        '?tag=' + this.tag +
                        '&before=' + this.lastEntryIndex +
                        '&take=' + this.entriesPerRequest +
                        '&family_hash=' + this.familyHash
                ).then(response => {
                    this.lastEntryIndex = 0 //response.data.entries.length ? _.last(response.data.entries).sequence : this.lastEntryIndex;
                    this.hasMoreEntries = response.data.entries.length >= this.entriesPerRequest;
                    //this.recordingStatus = false //response.data.status;
            ...
            updateEntries(){
                ...
                this.updateEntriesTimeout = setTimeout(() => {
                    ...
                    if (uuids.length) {
                        axios.post(Microscope.apiPath +  this.resource, {
                            uuids: uuids
                        }).then(response => {
                            this.recordingStatus = response.data.status;
...
<template>
    <div class="card">
        ...
        <p v-if="recordingStatus !== 'enabled'" class="mt-0 mb-0 disabled-watcher d-flex align-items-center">
            ...
            <span class="ml-1" v-if="recordingStatus == 'disabled'">Microscope is currently disabled.</span>
            <span class="ml-1" v-if="recordingStatus == 'paused'">Microscope recording is paused.</span>
        ...
        <table id="indexScreen" class="table table-hover table-sm mb-0 penultimate-column-right" v-if="ready && entries.length > 0">
            ...
            <transition-group tag="tbody" name="list">
                ...
                <tr v-for="entry in entries" :key="entry.name & entry.id & entry.job_name & entry">
